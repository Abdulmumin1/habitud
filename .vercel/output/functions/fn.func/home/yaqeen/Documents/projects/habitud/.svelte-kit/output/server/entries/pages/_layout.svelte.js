import { n as noop, a as assign, i as identity, c as create_ssr_component, b as subscribe, o as onDestroy, v as validate_component, m as missing_component, d as add_attribute, e as each, f as escape, g as null_to_empty, h as is_promise } from "../../chunks/ssr.js";
...
const css = {
  code: ".custom{--toastBackground:#fed7aa;--toastBarBackground:#fdba74;--toastColor:black}",
  map: null
};
const Layout = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  let webManifestLink;
  const options = { theme: {}, classes: ["custom"] };
  $$result.css.add(css);
  webManifestLink = pwaInfo ? pwaInfo.webManifest.linkTag : "";
  return `${$$result.head += `<!-- HEAD_svelte-1pmpekp_START --><!-- HTML_TAG_START -->${webManifestLink}<!-- HTML_TAG_END --><!-- HEAD_svelte-1pmpekp_END -->`, ""} ${slots.default ? slots.default({}) : ``} ${validate_component(SvelteToast, "SvelteToast").$$render($$result, { options }, {}, {})} ${function(__value) {
    if (is_promise(__value)) {
      __value.then(null, noop);
      return ``;
    }
    return function({ default: ReloadPrompt }) {
      return ` ${validate_component(ReloadPrompt, "ReloadPrompt").$$render($$result, {}, {}, {})} `;
    }(__value);
